...
public class QuorumPeer extends ZooKeeperThread implements QuorumStats.Provider {
     ...
     *
     * Reading:
     *  We can get the information from official documentation that {@link #initLimit} is timeouts ZooKeeper uses to limit the length of time the ZooKeeper servers in quorum have to connect to leader
     *
     * */
     ...
     *
     * Reading:
     *  We can get the information from official documentation that {@link #syncLimit} limits how far out of date a server can be from a leader
     *
     * */
    ...
    public synchronized void start() {
         ...
         *
         * Reading:
         *  5- Resume the {@link ZKDatabase} to latest data
         *
         * */
        loadDataBase();
         ...
         *
         * Reading:
         *  6 - Start a {@link ServerCnxnFactory}
         *
         * */
        cnxnFactory.start();
         ...
         *
         * Reading:
         *  7 - Set up some information which is used to elect
         *
         * */
        startLeaderElection();
    ...
    private void loadDataBase() {
		...
		try {
		    // Reading: Load the persisted data and build memory database
            /**
             *
             * Reading: The progress of load database is as followes:
             *  1. load snapshot and build datatree
             *  2. load transaction log
             *  3. apply the transaction logs to datatree
             *  4. commit the transaction logs and add them to committed log
             *  5. update the latest highest zxid
             *
             * */
            zkDb.loadDataBase();
             ...
             *
             * Reading:
             *  The zxid is composed by epoch and counter. Epoch will be changed when new election is finished, and counter is the number of transaction log
             *      zxid: {
             *          epoch 4bytes
             *          counter 4bytes
             *      }
             *
             *  Zxid is a 8 bytes string, and the highest 4 bytes is epoch, remaining 4 bytes is counter.
             *
             * */
            ...
            try {
                ...
                if (epochOfZxid > currentEpoch && updating.exists()) {
                    ...
                    // Reading: File `updatingEpoch` exists iff server was terminated after taking a snapshot but before updating current epoch.
                 ...
                 *
                 * Reading:
                 *  Set currentEpoch to the maximum epoch from snapshot and transaction logs.
                 *  But the data may be lost in this case.
                 *
                 * */
    ...
    synchronized public void startLeaderElection() {
    	try {
    	    // Reading: Always vote for itself.
        ...
        // Reading: Open socket which is used to connect between different ZooKeeper server when electionType is 0
        if (electionType == 0) {
            try {
    ...
    public void run() {
        ...
        try {
            ...
            while (running) {
                ...
                // Reading: Check my state
                switch (getPeerState()) {
                    /**
                     *
                     * Reading:
                     *  The initial state of server is always at LOOKING state.
                     *
                     * */
                case LOOKING:
                    LOG.info("LOOKING");
                     ...
                     *
                     * Reading:
                     *  If system property "readonlymode.enabled" exists, start a {@link ReadOnlyZooKeeperServer} when I'm partitioned
                     *  Else wait notifications until I recovery
                     *
                     * */
                    if (Boolean.getBoolean("readonlymode.enabled")) {
                        ...
                        Thread roZkMgr = new Thread() {
                            public void run() {
                                try {
                                    ...
                                    /**
                                     * Reading:
                                     *  Sleep for up to two seconds and we're partitioned if still at LOOKING state
                                     * */
                        ...
                        try {
                             ...
                             *
                             * Reading:
                             *  If I'm partitioned, method {@link #makeLEStrategy().lookForLeader()} will never return until exception occurred
                             *
                             * */
                             ...
                             *
                             * Reading:
                             *  Keep at LOOKING state although I'm partitioned to recovery in the future
                             *
                             * */
                             ...
                             *
                             * Reading:
                             *  Interrupt the {@link roZkMgr} and shutdown the {@link roZk} to prevent the server has recovery from failure but {@link ReadOnlyZooKeeperServer} is still used
                             *
                             * */
                            roZkMgr.interrupt();
                            roZk.shutdown();
                ...
                case OBSERVING:
                    try {
                         ...
                         *
                         * Reading:
                         *  Switch to LOOKING state if exception occurred
                         *
                         * */
                        setPeerState(ServerState.LOOKING);
                    }
